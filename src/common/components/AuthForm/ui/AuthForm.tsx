...
export const AuthForm: FC<AuthFormProps> = ({ title, onSubmit, validationSchema, link, errorMessage }) => {
  const {
    register,
    handleSubmit,
    formState: { errors }
  ...
  return (
    <div className='w-full bg-gray-800 flex justify-center items-center'>
      <div className='w-2/6 border-2 min-w-[300px] border-solid border-gray-500 rounded-3xl flex flex-col p-6 bg-gray-700'>
        ...
        {errorMessage && <span className='text-red-500 mb-10 h-[20px] text-center'>{errorMessage}</span>}
        <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col'>
          <InputField label='Email' type='text' name='email' error={errors.email} register={register} />
